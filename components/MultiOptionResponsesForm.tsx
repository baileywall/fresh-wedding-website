...
export function MultiOptionResponsesForm(
              ...
              <div class="flex flex-col">
                {event.options.map((option, index) => (
                  <div class="self-start">
                    <input
                      type="checkbox"
                      id={`${event.id}:${person.id}:MULTI_OPTIONS:${index}`}
                      name={`${event.id}:${person.id}:MULTI_OPTIONS`}
                      value={index}
                      checked={response?.options_response?.includes(index)}
                    />
                    <label
                      for={`${event.id}:${person.id}:MULTI_OPTIONS:${index}`}
                      class="mx-2"
                    >
                      {option}
                    </label>
